...
- **Population culling** – Each live cell has the configured per-generation chance to die, from `0%` to `0.10%`. Culling never directly removes the final live cell. Culled cells briefly flash black.
- **Spontaneous growth** – Each empty cell has the configured per-generation chance to sprout, from `0%` to `0.10%`. A sprout's chance to form a 2 × 2 block rises from `0%` in isolation to `10%` beside viable existing life, then declines with wrapped-grid distance. New sprouts briefly flash bright yellow.
